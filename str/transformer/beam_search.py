...
class SequenceBeamSearch(tf.Module):
    ...
    """
      ...
      """
            ...
            new_cache = tf.nest.map_structure(
                lambda t: _unflatten_beam_dim(t, batch_size, self.beam_size), flat_cache)
      ...
      """
            ...
            top_alive_seq, top_alive_log_probs, top_alive_cache = _gather_topk_beams(
                [new_seq, new_log_probs, new_cache], new_log_probs, batch_size, self.beam_size)
      ...
      """
            ...
            if not self.padded_decode:
                finished_seq = tf.concat(
                    [finished_seq,
                     tf.zeros([batch_size, self.beam_size, 1], tf.int32)], axis=2)
            ...
            top_finished_seq, top_finished_scores, top_finished_flags = (_gather_topk_beams(
                [finished_seq, finished_scores, finished_flags], finished_scores, batch_size,
                self.beam_size))
      ...
      """
            ...
            alive_state = _get_new_alive_state(new_seq, new_log_probs, new_finished_flags,
                                               new_cache)
    ...
    def _create_initial_state(self, initial_ids, initial_cache, batch_size):
        ...
        for key, value in initial_cache.items():
            for inner_value in tf.nest.flatten(value):
                if inner_value.dtype != self.dtype:
                    raise TypeError("initial_cache element for key '%s' has dtype %s that does not "
        ...
        alive_cache = tf.nest.map_structure(lambda t: _expand_to_beam_size(t, self.beam_size),
                                            initial_cache)
        ...
        finished_scores = tf.ones([batch_size, self.beam_size], dtype=self.dtype) * -inf(self.dtype)
        ...
        if self.padded_decode:
            state_shape_invariants = {
                ...
                _StateKeys.ALIVE_SEQ:
                    tf.TensorShape([batch_size, self.beam_size, self.max_decode_length + 1]),
...
def sequence_beam_search(symbols_to_logits_fn,
  ...
  """
    sbs = SequenceBeamSearch(symbols_to_logits_fn, vocab_size, beam_size, alpha, max_decode_length,
                             eos_id, padded_decode, dtype)
